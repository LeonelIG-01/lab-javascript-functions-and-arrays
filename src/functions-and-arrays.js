...
function avg(arr){
  ...
  const stuffValues = [];
  arr.forEach(function(eachThing){
    if(typeof eachThing == 'string'){
      stuffValues.push(eachThing.length);
    } else {
      stuffValues.push(Number(eachThing));
    }
  });
  return Number(averageNumbers(stuffValues).toFixed(2));
}
